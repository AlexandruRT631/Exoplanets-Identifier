...
const Home = () => {
  ...
  const sendCSV = async () => {
    ...
    // const resultWithTime = result.map((r: any, i: number) => ({ ...r, time: currentTime, originalInput: objects?.[0] }));
  ...
  const sendInput = async () => {
    await axios.post('http://127.0.0.1:8000/predict', [
      ...
      const currentTime = (new Date()).toISOString();
      const result = response.data.map((r: any) => ({ ...r, time: currentTime, originalInput: input }));
